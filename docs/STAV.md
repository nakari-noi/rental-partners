...
12. [ ] **Ďalší krok:** dohodnúť náhradu karty hodnotenia v hero → zapracovať schválené zmeny z plátna do kódu (`/impeccable harden` formulár, `colorize` kontrast, `layout` cenník, `onboard` kontakt, `quieter` hero, `clarify` texty, `polish` hlavička) → znova `site-check` + `critique`
